...
serve(async (req) => {
  ...
  try {
    ...
    try {
      trendData = await callAnthropicTool<Record<string, unknown>>({
        system: `You are a Singapore market intelligence analyst. The current date is ${today}. Provide current, realistic market trend data for Singapore's key business sectors as of TODAY. Base your analysis on the latest real market conditions, government policies (like Smart Nation initiatives, Budget 2026), and the most recent economic data. All quarterly momentum data MUST use the most recent 4 quarters leading up to the current date. Be specific to Singapore's context.`,
        userMessage: `Analyze Singapore's current market trends as of today (${today}) across these sectors: F&B, Fintech, E-commerce, Healthcare, Logistics, EdTech, CleanTech, and PropTech. Provide growth rates, saturation levels, risk scores, and quarterly momentum data for the most recent 4 quarters.`,
